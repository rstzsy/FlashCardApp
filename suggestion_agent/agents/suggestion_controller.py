...
class SuggestionController:
    ...
    def run(self, request: SuggestionRequest) -> SuggestionResponse:
        ...
        fsrs_records = self.db.get_fsrs_cards(user_id)
        user_sets = self.db.get_user_sets(user_id)
        ...
        # Build {setId: title} lookup từ user_sets
        set_title_lookup: dict[str, str] = {
            s.get("SetId", ""): s.get("Title", "")
            for s in user_sets
        }

        card_lookup = self.db.get_all_cards_for_user(user_id, set_ids)
        ...
        for card_id, card_data in card_lookup.items():
            ...
            scored.append({
                ...
                "topic": set_title_lookup.get(set_id, ""),
                ...
                "last_review": fsrs.get("lastReview"),
                "due": fsrs.get("due"),
            })
        ...
        user_ctx_for_gemini = {
            "englishLevel": user_profile_ext.get("englishLevel", "intermediate"),
            "interests":    user_profile_ext.get("interests", "general"),
            "streak":       user_profile.get("streak", 0),
            "targetGoal":   user_profile_ext.get("targetGoal", ""),
